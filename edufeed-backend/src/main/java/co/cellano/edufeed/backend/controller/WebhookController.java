...
public class WebhookController {
    ...
    @Operation(summary = "Webhook de pagos", description = "Recibe notificaciones de pagos desde caja/POS y almacena la transacción. Intenta conciliación automática por referencia externa.")
    ...
    @Operation(summary = "Listar transacciones", description = "Obtiene todas las transacciones con paginación.")
    ...
    public ResponseEntity<TransaccionCajaResponse> obtenerTransaccion(
            @Parameter(description = "ID de la transacción") @PathVariable("id") UUID id) {
        TransaccionCajaResponse transaccion = transaccionCajaService.obtenerPorId(id);
        return ResponseEntity.ok(transaccion);
    }

    @GetMapping("/transacciones/no-conciliadas")
    @Operation(summary = "Listar transacciones no conciliadas", description = "Obtiene todas las transacciones que aún no han sido conciliadas con un pago.")
    ...
    @Operation(summary = "Listar transacciones conciliadas", description = "Obtiene todas las transacciones que ya han sido conciliadas con un pago.")
    ...
    @Operation(summary = "Listar transacciones por proveedor", description = "Obtiene todas las transacciones de un proveedor específico (ej: 'WOMPI', 'MERCADOPAGO').")
    public ResponseEntity<List<TransaccionCajaResponse>> listarPorProveedor(
            @Parameter(description = "Nombre del proveedor de pagos") @PathVariable("proveedor") String proveedor) {
    ...
    @Operation(summary = "Listar transacciones por estado", description = "Obtiene transacciones filtradas por estado (PENDIENTE, APROBADO, RECHAZADO, ANULADO).")
    public ResponseEntity<List<TransaccionCajaResponse>> listarPorEstado(
            @Parameter(description = "Estado de la transacción") @PathVariable("estado") String estado) {
    ...
    @Operation(summary = "Listar transacciones por rango de fechas", description = "Obtiene transacciones recibidas en un rango de fechas específico.")
    public ResponseEntity<List<TransaccionCajaResponse>> listarPorRangoFechas(
            @Parameter(description = "Fecha de inicio (ISO 8601)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) OffsetDateTime desde,
            @Parameter(description = "Fecha de fin (ISO 8601)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) OffsetDateTime hasta) {
    ...
    @Operation(summary = "Conciliar transacción manualmente", description = "Concilia manualmente una transacción con un pago específico.")
    public ResponseEntity<TransaccionCajaResponse> conciliarManual(
            @Parameter(description = "ID de la transacción a conciliar") @PathVariable("id") UUID id,
    ...
    @Operation(summary = "Desconciliar transacción", description = "Revierte la conciliación de una transacción (útil si se concilió incorrectamente).")
    public ResponseEntity<TransaccionCajaResponse> desconciliar(
            @Parameter(description = "ID de la transacción a desconciliar") @PathVariable("id") UUID id) {
    ...
    @Operation(summary = "Estadísticas de conciliación", description = "Obtiene estadísticas generales de transacciones (total, conciliadas, no conciliadas, porcentaje).")
    ...
    @Operation(summary = "Contar transacciones no conciliadas", description = "Retorna la cantidad de transacciones pendientes de conciliación.")
