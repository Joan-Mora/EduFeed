...
public class ReportApiClient {
    ...
    public List<IngresosDiariosItem> ingresos(LocalDate desde, LocalDate hasta) throws IOException {
        ...
        if (desde != null) b.addQueryParameter("desde", desde.atStartOfDay().atOffset(OffsetDateTime.now().getOffset()).toString());
        if (hasta != null) b.addQueryParameter("hasta", hasta.plusDays(1).atStartOfDay().minusNanos(1)
    ...
    public byte[] exportCsv(String tipo, LocalDate desde, LocalDate hasta) throws IOException {
        // tipo: ingresos|asistencias|rechazos|derechos-activos (coinciden con endpoints CSV)
