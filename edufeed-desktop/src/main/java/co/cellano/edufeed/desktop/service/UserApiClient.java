...
public class UserApiClient {
    ...
    public PageResponse<UsuarioDto> listPaged(int page, int size, Filters f) throws IOException {
        // Estrategia: si hay documento -> devuelve página de 1; si hay nombre -> buscar y paginar en cliente; si no filtros -> usar paginado backend
        ...
        if (f != null && (f.tipo() != null || Boolean.TRUE.equals(f.soloActivos()))) {
        ...
        try (Response res = http.newCall(req).execute()) {
            ...
            return mapper.readValue(body, new TypeReference<PageResponse<UsuarioDto>>(){});
        }
    }
    ...
    private List<UsuarioDto> filtrarTipoYActivo(List<UsuarioDto> in, Filters f) {
        ...
        return in.stream()
                .filter(u -> f.tipo()==null || (u.tipoUsuario!=null && u.tipoUsuario.equalsIgnoreCase(f.tipo())))
    ...
    public List<UsuarioDto> listAll(boolean soloActivos) throws IOException {
        ...
        if (soloActivos) b.addQueryParameter("soloActivos", "true");
    ...
    public void desactivar(String id) throws IOException {
        Request req = authJson(new Request.Builder()
                ...
                .delete()).build();
        try (Response res = http.newCall(req).execute()) {
            if (res.code()!=204 && !res.isSuccessful()) throw new IOException("HTTP "+res.code());
        }
    }
    ...
    public static class PageResponse<T> {
        ...
        public static <T> PageResponse<T> of(List<T> content, long total, int totalPages, int number, int size) {
            ...
            p.content = content; p.totalElements = total; p.totalPages = totalPages; p.number = number; p.size = size;
            return p;
        }
    }
    ...
    public enum Modalidad { HUELLA, ROSTRO, VOZ, MANUAL }
