...
public class PaymentFormView extends VBox {
    ...
    private final ObjectProperty<BigDecimal> monto = new SimpleObjectProperty<>(new BigDecimal("0"));
    ...
    public PaymentFormView() { this(null,null,null); }

    public PaymentFormView(BigDecimal tarifaDiario, BigDecimal tarifaMensual, BigDecimal tarifaPaqueteDia) {
        setSpacing(8); setPadding(new Insets(12));
        this.tarifaDiario = tarifaDiario != null ? tarifaDiario : new BigDecimal(System.getenv().getOrDefault("DESKTOP_TARIFA_DIARIO","5000"));
        this.tarifaMensual = tarifaMensual != null ? tarifaMensual : new BigDecimal(System.getenv().getOrDefault("DESKTOP_TARIFA_MENSUAL","60000"));
        this.tarifaPaqueteDia = tarifaPaqueteDia != null ? tarifaPaqueteDia : new BigDecimal(System.getenv().getOrDefault("DESKTOP_TARIFA_PAQUETE_DIA","12000"));
        ...
        grid.add(new Label("Días (paquete):"), 0,r); grid.add(spDias,1,r++);
        grid.add(new Label("Monto:"), 0,r); grid.add(tfMonto,1,r++);
        grid.add(new Label("Método pago:"), 0,r); grid.add(cbMetodo,1,r++);
        grid.add(new Label("Referencia:"), 0,r); grid.add(tfReferencia,1,r++);
        ...
        spDias.disableProperty().bind(Bindings.createBooleanBinding(() -> tipoPago.get() != TipoPago.PAQUETE, tipoPago));

        // Cálculo de monto en tiempo real
        Runnable recalc = () -> {
            BigDecimal m;
            if (tipoPago.get() == TipoPago.DIARIO) m = tarifaDiario;
            else if (tipoPago.get() == TipoPago.MENSUAL) m = tarifaMensual;
            else m = tarifaPaqueteDia.multiply(BigDecimal.valueOf(Math.max(1, diasPaquete.get())));
            monto.set(m);
    ...
    private void submit() {
        lbError.setText("");
        if (monto.get().compareTo(BigDecimal.ZERO) <= 0) { lbError.setText("Monto inválido"); return; }
        if (cbMetodo.getValue() == null || cbMetodo.getValue().isBlank()) { lbError.setText("Seleccione método de pago"); return; }
        SubmitData data = new SubmitData(tipoPago.get(), diasPaquete.get(), monto.get(), cbMetodo.getValue(), tfReferencia.getText().trim(), cbAprobarAuto.isSelected());
        if (onSubmit != null) onSubmit.accept(data, () -> btnConfirmar.setDisable(false));
        btnConfirmar.setDisable(true);
    }

    public void setOnSubmit(BiConsumer<SubmitData, Runnable> onSubmit) { this.onSubmit = onSubmit; }

    public record SubmitData(TipoPago tipo, int diasPaquete, BigDecimal monto, String metodo, String referencia, boolean aprobarAuto) {}
}
